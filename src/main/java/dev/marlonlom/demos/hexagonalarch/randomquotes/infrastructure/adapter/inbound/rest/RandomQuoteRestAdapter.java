 ...
 * 
 * <p>
 * This controller bridges the web layer with the application/business layer by
 * invoking the {@link GetRandomQuoteUseCase} and mapping the domain model to a
 * REST response using {@link RandomQuoteResponseRestMapper}.
 * </p>
 * 
 * <p>
 * Follows the principles of Hexagonal Architecture by acting as an input
 * adapter.
 * </p>
 *
 ...
 */
@RestController
@RequestMapping(path = "/v1/quotes", produces = { MediaType.APPLICATION_JSON_VALUE })
...
public class RandomQuoteRestAdapter {
	 ...
	 *
	 * @return a {@link ResponseEntity} containing the {@link RandomQuoteResponse}
	 *         and HTTP status 200 (OK)
	 */
	@GetMapping(value = "/random")
	@Operation(tags = "get-random-quote", description = "Retrieves a random quote", summary = "Retrieves a random quote", responses = {
			@ApiResponse(responseCode = "400", description = "Random quote not found", content = {
					@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = RandomQuoteFailure.class)) }) })
	public ResponseEntity<RandomQuoteResponse> getRandomQuote() {
		RandomQuote dto = this.getRandomQuoteUseCase.getRandomQuote();
		return new ResponseEntity<>(this.randomQuoteRestMapper.toRandomQuoteResponse(dto), HttpStatus.OK);
	}
}
