...
/**
 * External REST adapter that implements the {@link RandomQuoteOutboundPort} to
 * fetch random quotes from an external API or REST service. <br>
 * <br>
 * This class serves as the infrastructure implementation of the output port,
 * allowing the application to remain decoupled from specific REST client logic.
 * 
...
public class RandomQuoteExternalRestAdapter implements RandomQuoteOutboundPort {
	...
	public Optional<RandomQuote> getRandomQuote() {
		try {
			...
			ResponseEntity<RandomQuote> response = restTemplate.exchange(apiUrl, HttpMethod.GET,
					new org.springframework.http.HttpEntity<>(headers), RandomQuote.class);
